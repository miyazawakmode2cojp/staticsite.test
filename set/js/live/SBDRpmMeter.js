...
var svg = d3.select("#rpm_meter").append("svg")
 .attr("width", width)
 .attr("height", height);
...
// 目盛の追加
// 0 を10000個の500間隔で配列を作成
var ticks = d3.range(0, 10500, 500);
// グループ要素を目盛数分追加する
...
ticks_g.append("text")
  ...
  .text(function(d){ if(d % 1000 == 0 ){ return d/1000; }});
...
label = ticks_g.append("text")
  ...
  .attr("fill","#00f8d8")
  .attr("font-size",MnumberSize*2.66+"px")
...
// 単位表示
 ticks_g.append("text")
  ...
  .style("text-anchor","middle")
  .attr("fill","white")
  .attr("font-size",MnumberSize*1.1+"px")
  .attr("font-family", "sans-serif")
  .text("rpm")
...
// メータの更新
SBDRpmMeter.prototype.update = function(value) {
  //数値の更新
  label.text(value);
